...
class _UnexpandedRootTreeNode(_RootTreeNode):
    ...
    def _expand(self):
        sgf_grammar.make_tree(
            self._coarse_tree, self, TreeNode, TreeNode._add_child)
        delattr(self, '_coarse_tree')
        self.__class__ = _RootTreeNode
...
class SgfGame:
    ...
    def from_coarse_game_tree(cls, coarse_game, override_encoding=None):
        ...
        game.root = _UnexpandedRootTreeNode(game, coarse_game)
    ...
    def main_sequence_iter(self):
        ...
        """
        if isinstance(self.root, _UnexpandedRootTreeNode):
